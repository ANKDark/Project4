...
class ProfileController extends Controller
{
    ...
    public function updateUser(Request $request)
    {
        ...
        if (!$user->email_verified_at) {
            return redirect()->route('profile.blog', ['id' => $user->id])
                ->withErrors(['email_verified_at' => 'Bạn cần xác minh email trước khi cập nhật.']);
        }

        if (!Hash::check($request->password, $user->password)) {
            return redirect()->route('profile.blog', ['id' => $user->id])
                ->withErrors(['password' => 'Mật khẩu sai']);
        }
    ...
    public function profileblog(int $id)
    {
        ...
        return Inertia::render('Profile/ProfileBlog', [
            'user' => $user,
    ...
    public function userCreatePost(Request $request)
    {
        ...
        if (!$user->email_verified_at) {
            return redirect()->route('profile.blog', ['id' => $user->id])
                ->withErrors(['email_verified_at' => 'Bạn cần xác minh email trước khi thực hiện.']);
        }
        ...
        if ($profilepost->save()) {
            return redirect()->route('profile.blog', ['id' => $IdUser]);
        } else {
            return redirect()->route('profile.blog', ['id' => $IdUser])
                ->withErrors(['error' => 'Không thể lưu bài viết']);
        }
    }
