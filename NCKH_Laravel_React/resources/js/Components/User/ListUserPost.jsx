...
export default function ListUserPost({ user, post, interactionPosts, currentUserId, handleInteraction,
  ...
  const ReplyCommentPost = commentPostsThroughProfilePosts.flatMap(comment => comment.comment_post_details_reply || []);

  const interactionsCommentPostCount = commentPostsThroughProfilePosts.reduce((acc, comment) => {
    const id = comment.Id;
    ...
    <div className="card p-3 shadow-sm bg-crp mt-3">
      <div className="d-flex align-items-center justify-content-between">
        <div className="d-flex align-items-center">
          ...
          <div>
            <Link href={`/profile/${user.id}`} className="text-decoration-none fw-bold text-white">
              {user.name}
            </Link>
            <div>
